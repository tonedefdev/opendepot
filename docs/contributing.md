...
The `oidc-deploy` target configures `server.oidc.dexProxy.enabled: true`, so Dex is reverse-proxied through the server itself — the [recommended](configuration/oidc/proxy.md#proxy-dex-through-the-server) setup. `make oidc-forward` port-forwards only the server (`localhost:8080`); Dex is reachable at `/dex/*` through that same port-forward, so there's no need to also forward Dex directly.
