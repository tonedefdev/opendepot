...
# Start Next.js standalone server in the background, bound to loopback only
# so that only the nginx upstream on 127.0.0.1:3000 can reach it.
HOSTNAME=127.0.0.1 node /app/server.js &
